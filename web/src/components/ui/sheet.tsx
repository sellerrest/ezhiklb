...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-background p-6 shadow-lg outline-none transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out',
  {
    ...
    compoundVariants: [
      ...
      { side: 'left', size: 'wide', class: 'w-full sm:max-w-2xl lg:max-w-4xl' },
      { side: 'right', size: 'wide', class: 'w-full sm:max-w-2xl lg:max-w-4xl' },
    ],
    defaultVariants: {
      side: 'right',
      size: 'default',
...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(({ side = 'right', size = 'default', className, children, ...props }, ref) => (
  <SheetPortal>
    <SheetOverlay />
    <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side, size }), className)} {...props}>
      <SheetPrimitive.Close className="focus:ring-ring data-[state=open]:bg-secondary absolute inset-e-4 top-4 cursor-pointer rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:outline-none disabled:pointer-events-none">
        <X className="h-4 w-4" />
