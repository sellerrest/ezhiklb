...
export function bindingRuleSummary(binding: Binding): string {
  if (binding.groups.length === 0) return "по умолчанию"
...
export function bindingStrip(binding: Binding, inbounds: Inbound[], outbounds: Outbound[]): string {
  ...
  const destination = binding.action === "drop" ? "Drop" : binding.targets.map((t) => outbounds.find((o) => o.id === t.outbound_id)?.name || "?").join(" + ") || "?"
  return `${inbound?.name || "?"} -> ${bindingRuleSummary(binding)} -> ${destination}`
}
